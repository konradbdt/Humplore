# Moderation Delta Specification

## ADDED Requirements

### Requirement: Report Visible Questions

The system SHALL allow a logged-in user to report a visible question without changing the question's visibility.
...
- **THEN** the system stores a report for that user and question
- **AND** the report status is `open`
...
The system SHALL allow a logged-in user to report a visible comment without changing the comment's visibility.
...
- **THEN** the system stores a report for that user and comment
- **AND** the report status is `open`
...
The system SHALL keep reporting isolated from existing post, question, comment, and engagement flows.
...
- **AND** existing `Neues gelernt!`, `Kommentieren`, `Merken`, and `Teilen` post actions remain available
